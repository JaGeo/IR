...
class IRTest(unittest.TestCase):
    def setUp(self):
        self.IR_calc = IR(
            PoscarName=os.path.join(path_here, "POSCAR"),
        ...
        self.IR_calc2 = IR(
            PoscarName=os.path.join(path_here, "POSCAR.NaCl"),
            ForceConstants=False,
        ...
        self.IRFC = IR(
            PoscarName=os.path.join(path_here, "POSCAR_Methanol"),
            ForceConstants=True,
    ...
    def test_attributes(self):
        ...
        self.assertAlmostEqual(
            abs(self.IR_calc._EigFormat[15, 47, 0]), 0.00084433323436
        )
        self.assertAlmostEqual(
            abs(self.IR_calc._EigFormat[15, 47, 1]), 0.00084433323436
        )
        self.assertAlmostEqual(
            abs(self.IR_calc._EigFormat[15, 47, 2]), 0.37170414232138
        )
        # check if sign of eigenvectors is consistent!!
        self.assertEqual(
            np.sign(self.IR_calc._EigFormat[14, 47, 2]),
        ...
        self.assertAlmostEqual(
            self.IR_calc_masses._frequencies[47], 2598.2875793589, places=1
        )
        # test calculation of eigenvectors
        self.assertAlmostEqual(
            abs(self.IR_calc_masses._EigFormat[15, 47, 0]), 0.00378948635566
        )
        self.assertAlmostEqual(
            abs(self.IR_calc_masses._EigFormat[15, 47, 1]), 0.00378948635566
        )
        self.assertAlmostEqual(
            abs(self.IR_calc_masses._EigFormat[15, 47, 2]), 0.33223420830758
        )
        # check if sign of eigenvectors is consistent
        self.assertEqual(
            np.sign(self.IR_calc_masses._EigFormat[14, 47, 2]),
        ...
        # test intensities
        self.assertAlmostEqual(
            self.IR_calc_masses.get_intensities()[-1], 1.1658569108600518
        )
